...
1. **Audit phase**: confirm the existing push isn't lossy. Bridge-server logs failures via `log.Printf` but doesn't retry. Add reconciliation: for every event written to bridge-server's `events` table, verify it landed in log-store with the same shape. Run a one-shot diff script over a recent session window.
2. **Backfill any gaps**: replay missing events from bridge-server's table into log-store. Repeat until parity.
3. **Read-path audit**: enumerate every place bridge-server reads from its `events` table (SSE replay endpoint, `MaterializedMessage` rendering, `RecoverInFlightTurn`, `HarnessToBridgeMap`, `InFlightTurnState` recovery). Confirm log-store can serve each query — extending log-store's API where needed before the read switch.
4. **Read switch**: route bridge-server's read paths to log-store's HTTP API. Bridge-server still writes to both tables.
...
6. **Stop dual-write**: bridge-server stops inserting into its own `events` table.
7. **Drop the table**: only after the verification window, drop bridge-server's `events` table and the related internal types (`EventWithID`, `InFlightTurnState`, `ToolUseBinding`, `TurnText`).
